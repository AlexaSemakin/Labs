...
std::string HafmanTree::Node::all_chr() const
{
    std::string res = "";
    for (unsigned int i = 0; i < _chr.size(); i++)
    {
        if (_chr[i]) {
            res +=  (char)i;
        }
...
{
    std::list<Node*> nodes;
    if (!build_list(input_text_file, nodes)){
        QMessageBox msgBox;
        msgBox.setText("File not found ;D");
        msgBox.exec();
        return;
    }
...
bool HafmanTree::build_list(const std::string& file_name, std::list<HafmanTree::Node*>& nodes)
{
