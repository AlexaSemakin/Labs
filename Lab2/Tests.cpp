...
void TreeTester::remove()
{
    ...
    while (!node_keys.empty()) {
        int random_index = rand() % node_keys.size();

        tree->delete_by_key(node_keys[random_index]);
        node_keys.erase(node_keys.begin() + random_index);
        check_remove(*tree, node_keys.size());
    }
...
void TreeTester::clear()
{
    ...
    check_clear(*tree2, _max_size - node_number + 1);
...
void TreeTester::min_key()
{
    ...
    min_val = tree->min_key();
    assert(min_val == min_key);
    deallocate_tree(tree);
}
...
void TreeTester::max_key()
{
    ...
    int max_val = tree->max_key();
    assert(max_val == _max_size - 1);
    ...
    max_val = tree->max_key();
    assert(max_val == max_key);
    deallocate_tree(tree);
}
...
void TreeTester::check_sum(const BinTree& tree, const int sumOfnode_keys)
{
    assert(tree.sum() == sumOfnode_keys);
}
...
{
    std::vector<int> node_keys = tree.get_keys();
    for (int i = 0; i < node_keys.size() - 1; i++)
        assert(node_keys[i] <= node_keys[i + 1]);
}
